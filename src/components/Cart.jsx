...
const Cart = () => {
    ...
    const calculateTotal = () => {
        const total = cartItems.reduce((prev, curr) => prev + curr.price, 2)
        return total.toFixed(2)
    }

    const removeFromCart = (index) => {
        const newCart = [...cartItems].filter((item,itemIndex) => index !== itemIndex)
        setCartItems(newCart)
    }
    
    return (
        <section className='relative px-4 md:px-0 top-24'>
                <div className="grid sm:px-10 lg:grid-cols-2 lg:px-20 xl:px-32">
                <div className="px-0 pt-8 lg:px-4">
                    <p className="text-xl font-medium">Order Summary</p>
                    <div className="px-2 py-4 mt-8 space-y-3 bg-white border rounded-lg sm:px-6">
                        {cartItems && cartItems.map((item,index) => (
                            <div className="flex items-center gap-4 py-3 bg-white rounded-lg">
                                    <Image height={500} width={500} className='w-1/2 aspect-video' src={item.image}></Image>
                                    <div className='flex flex-col gap-2 items-left'>
                                        <p className="text-lg font-semibold">{item.name}</p>
                                        <div className='flex items-center gap-4 md:flex-col md:items-start md:gap-2'>
                                            <p>{item.price}$</p>
                                            <ImBin onClick={() => removeFromCart(index)} className='cursor-pointer hover:text-red-500'/>
                                        </div>
                                    </div>
                            </div>
                        ))}
                ...
                <div className="px-4 pt-8 mt-10 bg-gray-50 lg:mt-0">
                        ...
                        <>
                            <div className="py-2 mt-6 border-t border-b">
                                <div className="flex items-center justify-between">
                                    ...
                                    <p className="font-semibold text-gray-900">${calculateTotal()-2}</p>
                                </div>
                                <div className="flex items-center justify-between">
                    ...
                    <button className="w-full px-6 py-3 mt-4 mb-8 font-medium text-white bg-gray-900 rounded-md" disabled={cartItems && cartItems.length === 0 ? true : false}>Place Order</button>
